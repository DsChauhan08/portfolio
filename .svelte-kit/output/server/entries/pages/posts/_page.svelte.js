...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<div class="mx-auto max-w-prose px-4 py-8 sm:px-6 lg:px-8"><h1 class="text-text mb-8 text-3xl font-bold tracking-tight">Posts</h1> `);
    if (data.posts.length) {
      ...
      for (let $$index_1 = 0, $$length = each_array.length; $$index_1 < $$length; $$index_1++) {
        ...
        config ? config.split(" ").map((cfg) => ({ colored: cfg.endsWith("c") })) : titleWords.map(() => ({ colored: false }));
        $$renderer2.push(`<a${attr("href", `/posts/${post.slug}`)} class="block"><article class="space-y-2"><h2 class="text-text hover:text-accent text-xl font-semibold"><!--[-->`);
        ...
        for (let i = 0, $$length2 = each_array_1.length; i < $$length2; i++) {
          ...
          $$renderer2.push(`<span${attr_style(`view-transition-name: ${stringify(getViewTransitionName(post.slug, word, i, titleWords))};`)}>${escape_html(word)}${escape_html(i < titleWords.length - 1 ? " " : "")}</span>`);
        }
        $$renderer2.push(`<!--]--></h2> `);
        if (post.metadata.published_at) {
          $$renderer2.push("<!--[-->");
          $$renderer2.push(`<p class="text-subtext0 text-xs">${escape_html(formatDate(post.metadata.published_at))}</p>`);
        } else {
          $$renderer2.push("<!--[!-->");
          $$renderer2.push(`<p class="text-subtext0 text-xs italic">Draft</p>`);
        }
        $$renderer2.push(`<!--]--> <p class="text-subtext1 text-sm">${escape_html(post.metadata.description)}</p> `);
        PostTags($$renderer2, { post, showIcon: false, colored: false });
        $$renderer2.push(`<!----></article></a>`);
      }
      $$renderer2.push(`<!--]--></div>`);
    } else {
      $$renderer2.push("<!--[!-->");
      $$renderer2.push(`<p class="text-subtext0">No posts yet.</p>`);
    }
    $$renderer2.push(`<!--]--></div>`);

import { a3 as ensure_array_like, a4 as attr, a9 as attr_style, aa as stringify, a5 as escape_html, ad as bind_props } from "../../../chunks/index2.js";
...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    if (data.philosophy && data.philosophy.length) {
      ...
      const each_array = ensure_array_like(data.philosophy);
      for (let $$index_1 = 0, $$length = each_array.length; $$index_1 < $$length; $$index_1++) {
        let entry = each_array[$$index_1];
        const titleWords = words(entry.metadata.title.text);
        $$renderer2.push(`<a${attr("href", `/philosophy/${entry.slug}`)} class="block"><article class="space-y-2"><h2 class="text-text hover:text-accent text-xl font-semibold"><!--[-->`);
      ...
      $$renderer2.push("<!--[!-->");
      $$renderer2.push(`<p class="text-subtext0">No thoughts recorded yet.</p>`);
    }
    $$renderer2.push(`<!--]--></div>`);

...
export function SidebarSubtitle({ children, secondary }: { children: React.ReactNode; secondary?: boolean; }) {
	return (
		<h2 className={cn("text-3xl font-semibold text-nowrap", jetbrainsMono.className, secondary ? "text-slate-500" : "text-slate-400")}>
			{children}
		</h2>
	)
}
...
export function SidebarNavLink({ children, href }: { children: React.ReactNode; href: string; }) {
	return (
		<a className="text-3xl transition-colors duration-200 font-semibold text-slate-500 hover:text-sky-400 text-nowrap" href={href} target="_blank" rel="noopener" referrerPolicy="no-referrer">
			{children}
		</a>
	)
}

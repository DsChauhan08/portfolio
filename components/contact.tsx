...
export function ContactLink({ icon, title, href }: { icon: React.ReactNode; title: string; href: string; }) {
	return (
		<>
			...
			<a className="w-full flex flex-row items-center experience-content py-4 gap-4 hover:text-sky-400 transition-colors duration-200" href={href} target="_blank" referrerPolicy="no-referrer" rel="noopener">
				{icon}
				<span className="font-semibold text-2xl text-slate-400">{title}</span>
			</a>
		</>
	);
}
